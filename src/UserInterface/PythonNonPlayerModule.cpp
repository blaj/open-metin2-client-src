...
PyObject* nonplayerGetMonsterDropList(PyObject* poSelf, PyObject* poArgs)
{
	int iVnum;
	if (!PyTuple_GetInteger(poArgs, 0, &iVnum))
		return Py_BuildException();

	const std::vector<CPythonNonPlayer::TMonsterDrop>* pDrops =
		CPythonNonPlayer::Instance().GetDrops(iVnum);

	PyObject* poList = PyList_New(0);

	if (pDrops)
	{
		for (const auto& drop : *pDrops)
		{
			PyObject* poDrop = Py_BuildValue("(ii)", drop.dwItemVnum, drop.wCount);
			PyList_Append(poList, poDrop);
			Py_DECREF(poDrop);
		}
	}
...
void initNonPlayer()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "GetMonsterDropList",			nonplayerGetMonsterDropList,		METH_VARARGS },
